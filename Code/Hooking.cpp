...
namespace VX
{
	...
	void Hooks::GetStatInt(rage::scrNativeCallContext* src)
	{
		...
		if (m_CheatRunning && g_HookFrameCount != *g_GameVariables->m_FrameCount)
		{
	...
	Hooking::~Hooking() noexcept
	{
		...
		g_LogMenu->Info("Disabled all Hooks!");
		MH_Uninitialize();
	}
